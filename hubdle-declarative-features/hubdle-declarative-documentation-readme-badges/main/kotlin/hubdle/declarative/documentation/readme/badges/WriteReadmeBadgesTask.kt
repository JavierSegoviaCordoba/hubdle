...
public abstract class WriteReadmeBadgesTask @Inject constructor(private val layout: ProjectLayout) :
    ...
    private val shieldsIoUrl: String = "https://img.shields.io"

    private val buildKotlinVersionBadge: String
        get() =
            "![Kotlin version]" +
                "($shieldsIoUrl/badge/kotlin-${kotlinVersion.get()}-blueviolet?logo=kotlin&logoColor=white)"
    ...
    private fun buildMavenRepoBadge(mavenRepo: MavenRepo): String {
        ...
        return if (label.contains("snapshot", ignoreCase = true)) {
            ...
            "[![$label]" +
                "($shieldsIoUrl/maven-central/v/${projectGroup.get()}/$namePath?label=$labelPath)]" +
