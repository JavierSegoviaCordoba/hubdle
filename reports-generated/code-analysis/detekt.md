...
* 213 number of kt files

## Complexity Report

* 13,153 lines of code (loc)

* 10,829 source lines of code (sloc)

* 7,104 logical lines of code (lloc)
...
* hubdle-project-gradle-plugin/main/kotlin/com/javiersc/hubdle/project/extensions/config/publishing/_internal/configureMavenPublishing.kt:74:41
```
...
72     }
73 
74 private fun HubdleConfigurableExtension.configurePublishingExtension() =
!!                                         ^ error
75     with(project) {
76         configure<PublishingExtension> {
77             configureRepositories(this)
...
* hubdle-project-gradle-plugin/main/kotlin/com/javiersc/hubdle/project/extensions/config/analysis/HubdleConfigAnalysisExtension.kt:203:17
```
This comment contains 'TODO:' that has been defined as forbidden in detekt.
```
```kotlin
200                     "${project.buildDir}/reports/kover/report.xml"
201                 )
202 
203                 // TODO: use `includes` and `excludes` from Hubdle when Detekt aggregated reports is
!!!                 ^ error
204                 //  fixed: https://github.com/detekt/detekt/issues/5041
205                 properties.property("sonar.sources", project.kotlinSrcDirs.get())
206                 properties.property("sonar.tests", project.kotlinTestsSrcDirs.get())

```

* hubdle-project-gradle-plugin/main/kotlin/com/javiersc/hubdle/project/extensions/config/binary/compatibility/validator/HubdleConfigBinaryCompatibilityValidatorExtension.kt:48:13
```
This comment contains 'TODO:' that has been defined as forbidden in detekt.
```
```kotlin
...
46 
47         configurable {
48             // TODO: Change this configurable with 0.7.0 as it must be applied on each project
!!             ^ error
49             check(isRootProject) {
50                 "`binaryCompatibilityValidator` must be applied only on root project"
51             }

```

* hubdle-project-gradle-plugin/main/kotlin/com/javiersc/hubdle/project/extensions/config/publishing/_internal/configureMavenPublishing.kt:152:25
```
This comment contains 'TODO:' that has been defined as forbidden in detekt.
```
```kotlin
149                             com.javiersc.hubdle.project.HubdleProperty.Publishing.nonSemver
150                         )
151                     check(isSemver || publishNonSemver) {
152                         // TODO: inject `$version` instead of getting it from the `project`
!!!                         ^ error
153                         """|Only semantic versions can be published (current: $version)
154                            |Use `"-Ppublishing.nonSemver=true"` to force the publication 
155                         """

```

* hubdle-project-gradle-plugin/main/kotlin/com/javiersc/hubdle/project/extensions/config/testing/HubdleConfigTestingExtension.kt:98:17
```
This comment contains 'TODO:' that has been defined as forbidden in detekt.
```
```kotlin
95              testReport.configure { task ->
96                  task.group = VERIFICATION_GROUP
97                  task.destinationDirectory.set(file("$buildDir/reports/allTests"))
98                  // TODO: Remove allprojects as it is incompatible with project isolation
!!                  ^ error
99                  task.testResults.from(allprojects.map { it.tasks.withType<Test>() })
100             }
101         }
...
* hubdle-project-gradle-plugin/main/kotlin/com/javiersc/hubdle/project/extensions/config/binary/compatibility/validator/HubdleConfigBinaryCompatibilityValidatorExtension.kt:72:21
```
Private property `apiValidationExtension` is unused.
```
```kotlin
69     HubdleConfigBinaryCompatibilityValidatorExtension
70     get() = getHubdleExtension()
71 
72 private val Project.apiValidationExtension: ApiValidationExtension
!!                     ^ error
73     get() = getHubdleExtension()
74 
...
generated with [detekt version 1.22.0](https://detekt.dev/) on 2023-03-11 21:40:02 UTC
