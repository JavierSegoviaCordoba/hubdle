...
* 254 number of classes

* 78 number of packages

* 206 number of kt files

## Complexity Report

* 11,681 lines of code (loc)

* 9,663 source lines of code (sloc)

* 6,566 logical lines of code (lloc)

* 52 comment lines of code (cloc)

* 1,228 cyclomatic complexity (mcc)

* 494 cognitive complexity

* 96 number of total code smells
...
* hubdle-gradle-plugin/main/kotlin/com/javiersc/hubdle/extensions/kotlin/multiplatform/_internal/configureMultiplatformAndroid.kt:34:51
```
In most cases using a spread operator causes a full copy of the array to be created before calling a method. This may result in a performance penalty.
```
```kotlin
31                         androidState.library.publishLibraryVariants.isNotEmpty() -> {
32                             val variants =
33                                 androidState.library.publishLibraryVariants.toTypedArray()
34                             publishLibraryVariants(*variants)
!!                                                   ^ error
35                         }
36                         androidState.library.allLibraryVariants -> publishAllLibraryVariants()
37                     }

```

* hubdle-settings-gradle-plugin/main/kotlin/com/javiersc/hubdle/settings/HubdleSettingsPlugin.kt:61:29
```
In most cases using a spread operator causes a full copy of the array to be created before calling a method. This may result in a performance penalty.
```
```kotlin
58     val autoInclude = hubdleSettings.autoInclude
59 
60     if (autoInclude.isEnabled) {
61         autoInclude.includes(*extractedProjects().toTypedArray())
!!                             ^ error
62         autoInclude.includedBuilds(*extractedBuildProjects().toTypedArray())
63 
64         for (include in autoInclude.includableProjects) {

```

* hubdle-settings-gradle-plugin/main/kotlin/com/javiersc/hubdle/settings/HubdleSettingsPlugin.kt:62:35
```
In most cases using a spread operator causes a full copy of the array to be created before calling a method. This may result in a performance penalty.
```
```kotlin
...
!!             ^ error
49 
50             sourceSets.all { configDefaultAndroidSourceSets() }
...
!!                 ^ error
23 
24         project.tasks.register<WriteReadmeBadgesTask>(WriteReadmeBadgesTask.name).configure {
25             this.projectGroup.set(project.group.toString())

```

generated with [detekt version 1.21.0](https://detekt.dev/) on 2022-07-31 16:36:25 UTC
