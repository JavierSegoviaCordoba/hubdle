...
internal fun Changelog.merged(): Changelog {
    ...
    val versions: List<Changelog.Version> = versionsMap.map { (version, subVersions) ->
        val groups: List<Changelog.Version.Group> =
            (listOf(version) + subVersions).flatMap { it.groups.toList() }
    ...
    val references: List<Changelog.Reference> = references.filter {
        it.version in versions.map(Changelog.Version::version)
    }

    return Changelog(header, versions, references)
}

private fun Changelog.Version.extractVersionWithoutStage(): String = version.takeWhile {
    it.isDigit() || it == '.'
}
...
private fun Changelog.Version.hasFinal(finalVersions: Iterable<Changelog.Version>): Boolean =
    finalVersions.any {
        it.version == extractVersionWithoutStage()
    }
